...
// не определяются: их показывает только аудит. Внутренняя пометка стадии
// (stageForUsCopy в internal-copy.ts) в карту не входит: карта уходит в браузер,
// пометку берёт только файл брифа.
