...
const ok = async (): Promise<Sent> => ({ ok: true })
const fail = async (): Promise<Sent> => ({ ok: false, error: 'boom' })
/** Подставные отправки с типами транспорта: аргументы вызовов видны в mock.calls. */
const messageMock = (impl: () => Promise<Sent>) => vi.fn<BriefTransport['sendMessage']>(impl)
const documentMock = (impl: () => Promise<Sent>) => vi.fn<BriefTransport['sendDocument']>(impl)
const noSleep = async () => {}
...
describe('deliverBrief', () => {
  it('документ прошёл: больше ничего не шлём', async () => {
    const sendMessage = messageMock(ok)
    expect(await deliverBrief(payload, { sendDocument: documentMock(ok), sendMessage })).toEqual({ ok: true, via: 'document' })
    expect(sendMessage).not.toHaveBeenCalled()
  })

  it('документ не прошёл: сводка с пометкой и текст частями', async () => {
    const sendMessage = messageMock(ok)
    expect(await deliverBrief(payload, { sendDocument: documentMock(fail), sendMessage, sleep: noSleep })).toEqual({ ok: true, via: 'chunks' })
  ...
  it('перед каждой частью пауза, чтобы Telegram не отклонил поток сообщений', async () => {
    ...
    expect(await deliverBrief(longPayload, { sendDocument: documentMock(fail), sendMessage, sleep })).toEqual({ ok: true, via: 'chunks' })
  ...
  it('сводка ушла, часть текста нет: доставлено частично, не провал', async () => {
    ...
    const r = await deliverBrief(longPayload, { sendDocument: documentMock(fail), sendMessage, sleep: noSleep })
  ...
  it('сводка не ушла: ошибка с причинами, частей не шлём', async () => {
    const sendMessage = messageMock(fail)
    const r = await deliverBrief(payload, { sendDocument: documentMock(fail), sendMessage, sleep: noSleep })
